...
void processMesh(SkeletonObject& m, aiMesh* mesh, const aiScene* scene, map<string, int>& bone_id, ll mask)
{
...
void processNode(SkeletonObject& m, aiNode* node, const aiScene* scene, map<string, int>& bone_id, ll mask)
{
	for (unsigned int i = 0; i < node->mNumMeshes; i++)
	{
		aiMesh* mesh = scene->mMeshes[node->mMeshes[i]];
...
bool Importer::Import(SkeletonObject& m, const string& path, ll mask) {
