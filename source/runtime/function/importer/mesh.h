...
public:
	...
	MeshPatch(vector<vertex>& mVertices, vector<uint>& mIndices, vector<ImportTexHead>& mTextures, bool post_process = false)
	{
		parseVector(mVertices, mIndices, mTextures, post_process);
	}

	void parseVector(vector<vertex>& mVertices, vector<us>& mIndices, vector<ImportTexHead>& mTextures, bool post_process = false) {
		clear();
...
{
public:
	Array<ImportTexHead, 100>		mTextures;
	...
	void addTexture(const ImportTexHead& tex, int to_whom);
	...
	int queryTexture(const string& path) {
		for (int i = 0; i < mTextures.size(); ++i) {
			if (mTextures[i].mPath == path)return i;
		}
		return -1;
	}
